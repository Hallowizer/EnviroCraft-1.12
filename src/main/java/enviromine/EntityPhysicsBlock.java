...
public class EntityPhysicsBlock extends EntityFallingBlock implements IEntityAdditionalSpawnData
{
	...
	public EntityPhysicsBlock(World world, double x, double y, double z, IBlockState state, boolean update)
	{
		...
		EM_PhysManager.usedSlidePositions.add("" + MathHelper.floor(this.posX) + "," + MathHelper.floor(this.posZ));
	...
	public void onUpdate()
	{
		...
		} else
		{
			...
			if(!this.world.isRemote)
			{
				...
				if(this.fallTime == 1)
				{
					if(this.world.getBlockState(new BlockPos(i, j, k)).getBlock() != this.block && !isLandSlide)
					{
						this.setDead();
						return;
					}
					...
					@SuppressWarnings("unchecked")
					List<Entity> before = ((List<Entity>)this.world.getEntitiesWithinAABB(EntityItem.class, this.boundingBox.expand(1, 1, 1)));
					this.world.setBlockToAir(new BlockPos(i, j, k));
					@SuppressWarnings("unchecked")
					List<Entity> after = ((List<Entity>)this.world.getEntitiesWithinAABB(EntityItem.class, this.boundingBox.expand(1, 1, 1)));
				...
				try
				{
			        net.minecraft.util.math.AxisAlignedBB axisalignedbb = block.getCollisionBoundingBoxFromPool(this.world, i, j - 1, k);
			        if(axisalignedbb != null)
			        {
			        	List fallingBlocks = this.world.getEntitiesWithinAABB(EntityPhysicsBlock.class, axisalignedbb);
				...
				if(earthquake)
				{
					for(int jj = j; j >= MathHelper.ceil(motionY); j --)
					{
						if(jj <= 5)
						{
							this.setDead();
							return;
						} else if(this.world.getBlockState(new BlockPos(i, jj, k)).getMaterial() == Material.LAVA)
						{
							this.setDead();
							return;
						} else if(this.world.getBlockState(new BlockPos(i, jj, k)).getMaterial() == Material.WATER)
						{
							this.setDead();
							return;
						} else if(this.world.getBlockState(new BlockPos(i, jj, k)).getBlock() == Blocks.OBSIDIAN || this.world.getBlockState(new BlockPos(i, jj, k)).getBlock() == Blocks.COBBLESTONE)
						{
							if(this.world.getBlockState(new BlockPos(i, jj - 1, k)).getMaterial() == Material.LAVA)
							{
								this.setDead();
								return;
							}
				...
				if(this.onGround)
				{
					...
					if(this.world.getBlockState(new BlockPos(i, j, k)).getBlock() != Blocks.PISTON_EXTENSION)
					{
						this.setDead();
						
						if(!this.world.canPlaceEntityOnSide(Blocks.ANVIL, i, j, k, true, 1, (Entity)null, (ItemStack)null) && !EM_PhysManager.blockNotSolid(this.world, i, j, k, false))
						{
							j += 1;
						}
						
						if((this.block == Blocks.SNOW_LAYER && this.world.getBlockState(new BlockPos(i, j, k)).getBlock() == Blocks.SNOW_LAYER && this.world.getBlockMetadata(i, j, k) < 15) || (!this.isBreakingAnvil2 && this.world.canPlaceEntityOnSide(Blocks.ANVIL, i, j, k, true, 1, (Entity)null, (ItemStack)null) && !BlockFalling.func_149831_e(this.world, i, j - 1, k) && this.world.setBlock(i, j, k, this.block, this.meta, 3)))
						{
							if(this.block == Blocks.SNOW_LAYER && this.world.getBlockState(new BlockPos(i, j, k)).getBlock() == Blocks.SNOW_LAYER && this.world.getBlockMetadata(i, j, k) < 15)
							{
								if(this.world.getBlockMetadata(i, j, k) >= 14)
								{
									this.world.setBlockState(new BlockPos(i, j, k), Blocks.SNOW.getDefaultState());
								} else
								{
									this.world.setBlockMetadataWithNotify(i, j, k, this.world.getBlockMetadata(i, j, k) + 1, 3);
								}
							} else if(meta != this.world.getBlockMetadata(i, j, k))
							{
								this.world.setBlockMetadataWithNotify(i, j, k, meta, 2);
							}
							
							EM_PhysManager.schedulePhysUpdate(this.world, i, j, k, true, earthquake? "Quake" : "Collapse");
							...
							Block bSurface = null;
							if(this.world.getBlockState(new BlockPos(i, j - 1, k)).getBlock() != Blocks.AIR && this.world.getBlockState(new BlockPos(i, j - 1, k)).getBlock().getMaterial() != Material.AIR)
							{
								bSurface = this.world.getBlockState(new BlockPos(i, j -1 , k)).getBlock();
								this.world.playSoundEffect((double)((float)i + 0.5F), (double)((float)j + 0.5F), (double)((float)k + 0.5F), bSurface.stepSound.func_150496_b(), (bSurface.stepSound.getVolume() + 1.0F) / 2.0F, bSurface.stepSound.getPitch() * 0.5F);
							}
							
							if(bSurface == null || bSurface != this.block)
							{
								this.world.playSoundEffect((double)((float)i + 0.5F), (double)((float)j + 0.5F), (double)((float)k + 0.5F), this.block.stepSound.func_150496_b(), (this.block.stepSound.getVolume() + 1.0F) / 2.0F, this.block.stepSound.getPitch() * 0.5F);
							}
							
							if(this.field_145810_d != null && block instanceof ITileEntityProvider)
							{
								TileEntity tileentity = this.world.getTileEntity(new BlockPos(i, j, k));
				...
				} else if(this.fallTime > 100 && !this.world.isRemote && (j < 1 || j > 256) || this.fallTime > 600)
				{
	...
	public void fall(float distance, float damageMultiplier)
	{
		if(this.isAnvil2)
		{
			...
			if(i > 0)
			{
				...
				{
					damagesource = this.block == Blocks.ANVIL ? DamageSource.ANVIL : DamageSource.FALLING_BLOCK;
				}
				...
				while(iterator.hasNext())
				{
					...
					entity.attackEntityFrom(damagesource, (float)Math.min(MathHelper.floor((float)i * this.fallHurtAmount2), this.fallHurtMax2));
				}
